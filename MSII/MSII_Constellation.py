...
class MSII_Satellite:
    def __init__(self):
        print("Using default constructor.")
        self.orbit = MSII_Orbit(0, 0, 0, 0, 0, 0)
        
    def __init__(self, sat_obj):
        self.orbit = MSII_Orbit(
            a=sat_obj["a"],
            e=sat_obj["e"],
            i=sat_obj["i"],
            raan=sat_obj["raan"],
...
class MSII_Constellation:
    def __init__(self):
        print("Using default constructor. Kinda sus.")
        self.sats = []
        for i in range(NUM_SATS):
            self.sats.append(MSII_Satellite())
    ...
    def __init__(self, constellation_dict):
        ...
        for sat in constellation_dict["sats"]: # initialize all the satellites
            self.sats.append(MSII_Satellite(sat))
            
            
    def assess_budget(self) -> MSII_Budget_Summary: # create an object representing adherence to the budget
        ...
        return MSII_Budget_Summary(total_estimated_cost, total_certain_cost)
    
    def assess_coverage(self) -> MSII_Coverage_Summary:
        ...
        for i in range(POSITIONAL_TESTING_RESOLUTION):
            ...
            for sat in self.sats:
                sat : MSII_Satellite # type annotation, ignore
    ...
    def assess_orbit_viability(self): 
        ...
        return MSII_Orbit_Viability_Summary(orbit_tests) # put it in a nice organized report

    def assess_volume(self):
        return MSII_Volume_Summary(self.payload, self.structure)
    ...
    def get_concise_summary(self):
        ...
        return MSII_Concise_Constellation_Summary(orbit_summary, budget_summary, volume_summary)
    ...
    def get_comprehensive_summary(self):
        ...
        return MSII_Comprehensive_Constellation_Summary(orbit_summary,
                                                        budget_summary,
                                                        cover_summary, 
                                                        volume_summary)
